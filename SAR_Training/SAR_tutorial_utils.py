...
def load_locomotion_SAR():
    ...
    root_dir = os.path.join( current_dir, 'agents/locomotion/')
    print(root_dir)
    ica = joblib.load(os.path.join(root_dir, 'ica.pkl'))
    pca = joblib.load(os.path.join(root_dir, 'pca.pkl'))
    normalizer = joblib.load(os.path.join(root_dir, 'normalizer.pkl'))
...
def get_vid(name, env_name, seed, timenow, episodes, video_name, determ=False,
    ...
    if 'Leg' in env_name:
        camera = 'front'#'side_view'
    else:
        camera = 'front'
    
    model_num = '2023_11_25_09_31_47'
    all_rewards = []
    for i,__ in tqdm(enumerate(range(episodes))):
        ...
        model = PPO.load(f'./StepBalance/SAR/{name}_model_{env_name}_{seed}_{timenow}')
        #vec = VecNormalize.load(f'./StepBalance/SAR/{name}_env_{env_name}_{seed}_2023_11_25_09_31_46', DummyVecEnv([lambda: env]))

        vec = VecNormalize.load(f'./StepBalance/SAR/{name}_env_{env_name}_{seed}_{timenow}', DummyVecEnv([lambda: env]))
